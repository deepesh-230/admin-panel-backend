...
let ServiceProvidersService = class ServiceProvidersService {
    ...
    async approve(id, currentUser) {
        const existing = await this.getScopedOrThrow(id, currentUser);
        if (existing.businessVerificationStatus !== client_1.BusinessVerificationStatus.IN_PROGRESS &&
            existing.businessVerificationStatus !== client_1.BusinessVerificationStatus.VERIFIED) {
            throw new common_1.BadRequestException('User has not submitted business verification yet. Wait for MCA/DIN/GSTIN/NMC/PAN details.');
        }
        const provider = await this.prisma.serviceProvider.update({
            where: { id },
            data: {
                ...
                rejectedReason: null,
                isActive: true,
                businessVerificationStatus: client_1.BusinessVerificationStatus.VERIFIED,
    ...
    async reject(id, reason, currentUser) {
        ...
        const provider = await this.prisma.serviceProvider.update({
            ...
            data: {
                ...
                approvedAt: null,
                isActive: false,
                businessVerificationStatus: client_1.BusinessVerificationStatus.REJECTED,
    ...
    async assertUserOwnsProvider(userId, providerId) {
        const provider = await this.prisma.serviceProvider.findFirst({
            where: {
                id: providerId,
                OR: [
    ...
    async resolveStateIdForUser(userId, stateId, locationLabel, city) {
        if (stateId) {
        ...
        if (haystack.trim()) {
            ...
            const match = states.find((s) => haystack.includes(s.name.toLowerCase()) ||
                (s.code && haystack.includes(s.code.toLowerCase())));
            if (match)
                return match.id;
        }
        throw new common_1.BadRequestException('Could not determine state. Update your profile state or include a clear state in the location.');
    }
    async createForUser(userId, data) {
        await this.assertCategoryLinks(data.categoryId, data.subcategoryId);
        const resolvedStateId = await this.resolveStateIdForUser(userId, data.stateId, data.locationLabel, data.city);
        const provider = await this.prisma.serviceProvider.create({
            data: {
                name: data.name.trim(),
    ...
    async updateForUser(userId, id, data) {
        ...
        const nextSubcategoryId = data.subcategoryId === undefined ? existing.subcategoryId : data.subcategoryId;
        await this.assertCategoryLinks(nextCategoryId, nextSubcategoryId);
        let nextStateId = existing.stateId;
        if (data.stateId !== undefined ||
            data.locationLabel !== undefined ||
            data.city !== undefined) {
            nextStateId = await this.resolveStateIdForUser(userId, data.stateId || existing.stateId, data.locationLabel, data.city);
        }
        const provider = await this.prisma.serviceProvider.update({
            where: { id },
            data: {
    ...
    async submitBusinessVerificationForUser(userId, id, data) {
        const existing = await this.assertUserOwnsProvider(userId, id);
        if (existing.businessVerificationStatus === client_1.BusinessVerificationStatus.VERIFIED) {
            throw new common_1.BadRequestException('Business is already verified');
        }
        ...
        if (!mcaId && !din && !gstin && !nmcId && !panId) {
            throw new common_1.BadRequestException('Provide at least one of MCA ID, DIN, GSTIN, NMC ID, or PAN');
        }
        ...
        const provider = await this.prisma.serviceProvider.update({
            ...
            data: {
                ...
                nmcId,
                panId,
                businessVerificationStatus: client_1.BusinessVerificationStatus.IN_PROGRESS,
